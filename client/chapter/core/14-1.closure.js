...
const value2 = sum(1,2); //3
const value = sum;       // sum(a,b){return a+b}
      //함수의 실행값을 넘길수도 있지만 함수 본문 자체를 넘기는 것도 가능하다
      //넘기면 그 대상이 그 함수를 가진거마냥 호출가능하다
...
const result = first(); // 30 // 세컨드 함수 본문이 담긴다. 결국 result 는 함수가 된다!
result() // result 를 실행할 수 있게됨
      // 이것이 클로저
      // second()함수가 태어난 환경을 기억해서 안쪽함수에서 x와 y 를 사용할 수 있게됨
...
function counter(){
  ...
  return function(){
    console.log(++count); // 이렇게 해도 상관없음. 굳이 안의 함수의 이름을 만질 필요 없다
  }
    // function inner(){
    //   console.log(++count);
    // }

    // return inner //이너함수를 전달
}

//inner 함수 없을때
counter(); // 실행 후 가비지 컬렉터가 count를 수집하면서 없어짐. 
counter();
counter();
counter(); // 계속 1이다. /가비지 컬렉터가 count를 수집. 수집해서 새로 정의하니까.

const c1 = counter() //값이 증가함. 이것이 클로저. inner함수가 count를 참조하고 있기때문에 /c1 만 접근가능한 공간 가ㄱ져가지않음

const c2 = counter() // c1의 실행에 영향받지 않고 새로운 환경이 생성되기 에 다시 1처럼

const c3 = c1 // c1과 동일한 환경이라 연결되어서 숫자가 증가함
